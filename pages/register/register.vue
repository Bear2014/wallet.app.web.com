<template>
	<view class="login">
		...
		<navigation style="margin: 0 -50rpx;" :navigationText ="'绑定iPay账号'"></navigation>
		...
		<view class="login_information_bg">
			<view class="login_information">
				<view class="login_user" style="width: 100%;">
					<image style="width: 30rpx;" src="../../static/icons/icon_phone.png" mode="widthFix"></image>
					<input @input="phoneInput"  type="number" v-model="user.phone" placeholder="请输入手机号，用于绑定iPayApp" maxlength="11">
				</view>
				...
				<view class="login_user">
					...
					<input type="text" v-model="user.paw" :password="!lookPassword" placeholder="输入iPay登录密码" maxlength="20">
				...
				<view class="login_user" v-show="userExists == 0" >
					<image src="../../static/icons/icon_lock.png" mode="widthFix"></image>
			...
			methods: {
				...
				obtionPhoneCode() {
					...
					let params = {
						...
						bc_account:uni.getStorageSync('token_H5Bc_account'),
						game_platform_code:uni.getStorageSync('token_H5GameCode'),
					}
					// 请求获取短信验证码 
					that.$api.tokenTimeOut("/bindSmsV2", "POST", params).then(res => {
						
						if(res.code == 0) {
							that.userExists = res.data.userExists
							uni.showToast({
								title: "验证码已发送！" + res.data.notice,
								icon: "none",
								duration: 4000
							});
				...
				inspectSub() {
					...
					if(!this.user.phone && !this.user.paw) {
						...
						return 
					}else if(this.userExists == 0 && this.user.paw != this.user.rePaw){
						uni.showToast({
							title: "密码输入不一致！",
							icon: "none",
							duration: 1500
				...
				subRegistration() {
					...
					let params = {
						...
						password_confirmation: this.userExists == 0 ? this.user.rePaw : undefined, // 用户再次确认的密码
						gender: 1,
						bc_account:uni.getStorageSync('token_H5Bc_account'),
						game_platform_code:uni.getStorageSync('token_H5GameCode'),
					}
					...
					that.$api.tokenTimeOut("/user/appBindV2", "POST", params).then(res => {
						...
						if(res.code == 0) {
							...
							uni.setStorageSync('token_H5', res.data.token)
							//3.隐藏加载框
							uni.hideLoading();
