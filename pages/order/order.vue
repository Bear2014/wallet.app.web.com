	...
	<view class="order_page">
		...
		<view class="order_tab">
			...
			<view class="order_tab_screen">
				<view class="order_tab_screen_ord" v-for="(item, index) in ['全部', '进行中', '已完成', '已取消']" :key="index">
					<view :class="[statics == (index + 1) ? 'active' : '', 'common']" @tap="reStatics(index)">{{ item }}
					</view>
				</view>
			</view>
		</view>
		<swiper class="order_swiper" :current="org" :indicator-dots="indicatorDots" :autoplay="autoplay"
			:interval="interval" :duration="duration" @change="swiperChange" disable-touch="false">
			<swiper-item>
				<view class="swiper_bg">
					<view class="order_date_select" @tap="goHistory">
						<view class="date_select_show hisBtn">
					...
					<scroll-view class="order_scroll" scroll-y="true" @scrolltolower="scrollPage" v-show="showDataPage"
						:refresher-threshold="70" :refresher-enabled="true" refresher-background="#F4F3F3"
						:refresher-triggered="triggered" @refresherrefresh="refreshData(1)">
              ...
              <loding/>
						...
						<view class="order_scroll_order" @tap="goOrderInfo(item)" v-for="(item,index) in dataList"
							:key="index">
							...
							<view class="order_scroll_center">
								...
								<view class="order_scroll_center_icon">
									<view
										:class="['order_scroll_center_padding', item.sale_status == 'trade_finish' ? 'order_scroll_center_padding4' : item.sale_status == 'trade_cancel' ? 'order_scroll_center_padding2' : item.sale_status == 'trade_stop' ? 'order_scroll_center_padding3' : '']">
										<text>{{item.desc}}</text>
									</view>
									<image v-if="item.pay_type == 1" src="@/static/icons/icon_Alipay_order_pagezfb.png"
										mode=""></image>
									<image v-if="item.pay_type == 2" src="@/static/icons/icon_WeChat_order_pagewx.png"
										mode=""></image>
									<image v-if="item.pay_type == 3" src="@/static/icons/icon_UnionPay_bank.png"
										mode=""></image>
									<image v-if="item.pay_type == 4" src="@/static/icons/icon_usdt.png" mode=""></image>
								</view>
								<text class="scroll_center_time">{{ item.created_at }}</text>
							</view>
			...
			<swiper-item>
				<view class="swiper_bg">
					...
					<scroll-view class="order_scroll" scroll-y="true" @scrolltolower="scrollPage" v-show="showDataPage"
						:refresher-threshold="70" :refresher-enabled="true" refresher-background="#F4F3F3"
						:refresher-triggered="triggeredTwo" @refresherrefresh="refreshData(2)">
						<!-- <view v-slot="refresher">
              <loding/>
            </view> -->
      ...
      <uni-datetime-picker     v-model="range" type="daterange" @maskClick="maskClick" />
		...
		<uv-datetime-picker ref="datetimePicker" :maxDate="Date.now()" v-model="value" mode="year-month"
			@confirm="confirm" @close="closeSelect">
		</uv-datetime-picker>
...
<script>
	...
	export default {
		...
		methods: {
			...
			reStatics(states) {
				// 修改订单筛选状态
				this.statics = states + 1
				console.log('states', states, typeof(states));
				switch (states) {
					case 0:
						this.staticsType = 'all';
						break;
					case 1:
						this.staticsType = 'trade_ing';
						break;
					case 2:
						this.staticsType = 'trade_finish';
						break;
					case 3:
						this.staticsType = 'trade_cancel';
						break;
				}
				console.log('this.staticsType', this.staticsType);
				// 初始化数据
				this.dataList = []
				this.page = 1
				// 获取筛选后的订单数据
			...
			otpList() {
				...
				if (this.org == 0) {
					...
					that.$api.tokenTimeOut("/order/v2/sale", "POST", params).then(res => {
					...
					that.$api.tokenTimeOut("/order/v2/buy", "POST", params).then(res => {
		...
		.order_swiper {
			...
			.swiper_bg {
				...
				.hisBtn {
					...
					background-color: #0c2be1 !important;
