<template>
	<view class="changepaw">
		...
		<navigation :navigationText ="'设置支付密码'"></navigation>
		<view class="changepaw_bg">
			<!-- <view class="changepaw_input">
				...
				<input type="text" @input="fpNumInput" v-model="user.paw" placeholder="请输入旧密码" :password="!change_look.oldPaw" maxlength="6">
			...
			<view class="changepaw_input">
				<image class="changepaw_input_img" src="../../../static/icon_login password.png" mode="widthFix"></image>
				<input type="text" @input="fpNumInput2" v-model="user.newPaw" placeholder="请再次输入新密码" :password="!change_look.newPaw" maxlength="6">
			...
			<view class="changepaw_input" >
				<image class="changepaw_input_img" src="../../../static/icon_login password.png" mode="widthFix"></image>
				<input type="text" @input="fpNumInput3" v-model="user.reNewPaw" placeholder="请再次输入新密码" :password="!change_look.newPaw" maxlength="6">
				<image style="margin:0 0 15rpx 50rpx;" @tap="change_look.newPaw = !change_look.newPaw" v-if="change_look.newPaw" src="../../../static/icons/eye_1.png" mode=""></image>
				<image style="margin:0 0 15rpx 50rpx;" @tap="change_look.newPaw = !change_look.newPaw"  v-else src="../../../static/icons/eye_2.png" mode=""></image>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			determinePaw() {
				...
				if( this.user.newPaw.length == 0 || this.user.reNewPaw.length == 0) {
					uni.showToast({
						title: "请完整输入密码！",
						icon: "none",
						duration: 1500
				...
				// 	return 
				// }else 
				if(this.user.newPaw.length < 6 || this.user.reNewPaw.length < 6 ) {
					uni.showToast({
						title: "新密码小于6位！",
						icon: "none",
						duration: 1500
				...
				// 	return 
				// }else 
				if(this.user.newPaw != this.user.reNewPaw) {
					uni.showToast({
						title: "新密码两次输入不一致！",
						icon: "none",
						duration: 1500
			...
			subChange() {
				...
				let params = {
					...
					type:"trans_pass" //修改登入密码类型
				}
				// 提交修改信息
				that.$api.tokenTimeOut("/user/resetPayPwd", "POST", params).then(res => {
